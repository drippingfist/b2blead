  ...
  return (
    <div>
      {messages.map((message, index) => {
        ...
        return (
          <div key={index} className={`flex ${isUser ? "justify-start" : "justify-end"} mb-8 group`}>
            ...
            <div className={`flex flex-col ${isUser ? "items-start" : "items-end"} max-w-[60%] min-w-[120px]`}>
              ...
              {/* Timestamp */}
              <p className="text-xs mt-1 px-2 text-[#616161]">{formatMessageTime(message.created_at)}</p>
            </div>
...
export function MessagesView({
  ...
  const formatMessageTime = (timestamp: string) => {
    ...
    // Use the bot's timezone if available, otherwise use local timezone
    const timezone = botData?.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone
        ...
        <div className="mb-6">
          ...
          {selectedDate ? (
            ...
            <p className="text-[#616161]">
              ...
              {botData?.timezone && <span> [Timezone: {botData.timezone}]</span>}
            </p>
          )}
        </div>
