...
export default function ChatsPage() {
  ...
  const threadsPerPage = 50 // Number of threads to show per page
  ...
  const fetchThreads = async (filter: DateFilter = dateFilter, page: number = currentPage) => {
    setLoading(true)
    setError(null)
    try {
      console.log("🧵 PAGE: Fetching threads for bot_share_name:", selectedBot, "with filter:", filter, "page:", page)
      const { threads: threadsData, totalCount } = await getThreadsSimple(threadsPerPage, selectedBot, filter, page)
      console.log(`🧵 PAGE: Fetched ${threadsData.length} threads out of ${totalCount} total`)
  ...
  useEffect(() => {
    if (userAccess.role) {
      // Reset to first page when bot or date filter changes
      setCurrentPage(0)
      fetchThreads(dateFilter, 0)
    }
  }, [selectedBot, userAccess.role, dateFilter])

  // Handle page changes
  const handlePageChange = (newPage: number) => {
    setCurrentPage(newPage)
    fetchThreads(dateFilter, newPage)
  }
  ...
  return (
    <div>
      ...
      <ThreadsView
        ...
        onRefresh={() => fetchThreads(dateFilter, currentPage)}
        bots={bots}
